...
MIC_DATA_DIR = 'C:\\Users\\Urban\\Documents\\Fakulteta za Elektrotehniko\\AVMS\\AVMS-Projekt\\Data'
...
class mic_thread(threading.Thread):
    ...
    def run(self):
        ...
        test = os.listdir(MIC_DATA_DIR)

        for item in test:
            if item.endswith(".npy"):
                os.remove(os.path.join(MIC_DATA_DIR, item))
        ...
        with recording_mic.recorder(samplerate=samplerate) as mic:
            ...
            while(1):
                ...
                if i == data_buffer.shape[0]:
                    ...
                    MIC_TIME_DATA.append(time.time()-self.t0)
                    np.save(MIC_DATA_DIR+'/recorded_data_' + str(file), data, allow_pickle=True)
                    file += 1
                    i = 0
...
class Microphone():
    ...
    def read_data(self):
        ...
        path = Path(MIC_DATA_DIR+'/recorded_data_' + str(self.file_num) + '.npy')
        if path.is_file():
...
class Application():
    ...
    def mainloop(self):
        while (not self.shutdown) and (not EXIT):
...
if __name__ == '__main__':
    app_thr = app_thread('GUI',1000)
    mic_thr = app_thread('Mic',2000)
